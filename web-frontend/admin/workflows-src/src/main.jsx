...
function App() {
  ...
  const connectionError = useCallback((connection, ignoredEdgeId = '') => {
    ...
    if (sourceNode?.type === 'condition') {
      ...
      if (outgoing.some((edge) => edge.sourceHandle === connection.sourceHandle)) return `分支 ${connection.sourceHandle} 已连接`;
    }
    ...
    <div className="studio-main">
      <aside className="node-library">
        ...
        <div className="node-list">{Object.entries(NODE_META).map(([type, meta]) => {
          const Icon = meta.icon;
          return <button key={type} draggable onDragStart={(event) => { event.dataTransfer.setData('application/x-workflow-node', type); event.dataTransfer.effectAllowed = 'move'; }} onClick={() => addNode(type)}><span className={`lib-icon tone-${meta.tone}`}><Icon size={16}/></span><span><strong>{meta.label}</strong><small>{type === 'subworkflow' ? '嵌套已发布流程' : type === 'loop' ? '遍历集合并调用子流程' : type === 'condition' ? '双端口条件路由' : type === 'plugin' ? '搜索学术 API' : '拖入画布或点击添加'}</small></span></button>;
        })}</div>
