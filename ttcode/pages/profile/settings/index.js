...
Page({
  ...
  onLoad(options = {}) {
    tt.setNavigationBarTitle({ title: "账号设置" });
  ...
  getApiBaseUrl() {
    const app = getApp();
    return tt.getStorageSync("apiBaseUrl") || (app && app.globalData && app.globalData.apiBaseUrl) || "http://127.0.0.1:3000";
  },
    ...
    tt.request({
      ...
      success: (res) => {
        if (res.statusCode === 200 && res.data && res.data.code === "SUCCESS") {
          ...
          cached.email = email;
          cached.phone = phone;
          tt.setStorageSync("userData", cached);
        } else if (res.statusCode === 401) {
          auth.clearLoginInfo();
          auth.requireLogin(true);
        } else {
          tt.showToast({ title: (res.data && res.data.error) || "加载失败", icon: "none" });
        }
      },
      fail: (err) => {
        console.error("获取账号信息失败:", err);
        tt.showToast({ title: "网络异常，请稍后再试", icon: "none" });
      },
      complete: () => {
        tt.hideLoading();
      }
  ...
  saveBasicInfo() {
    ...
    if (!nickname) {
      tt.showToast({ title: "请输入用户名", icon: "none" });
      return;
    }

    if (email && !/^([a-zA-Z0-9_\-.+])+@([a-zA-Z0-9-]+\.)+[a-zA-Z]{2,}$/.test(email)) {
      tt.showToast({ title: "邮箱格式不正确", icon: "none" });
      return;
    }
    ...
    tt.request({
      ...
      success: (res) => {
        if (res.statusCode === 200 && res.data && res.data.code === "SUCCESS") {
          ...
          tt.showToast({ title: "资料已更新", icon: "success" });
          this.fetchProfile();
        } else {
          tt.showToast({ title: (res.data && res.data.error) || "更新失败", icon: "none" });
          this.setData({ savingBasic: false });
        }
      },
      fail: (err) => {
        console.error("更新资料失败:", err);
        tt.showToast({ title: "网络异常，请稍后重试", icon: "none" });
        this.setData({ savingBasic: false });
      }
    });
  },
  ...
  uploadAvatar(filePath) {
    ...
    tt.getImageInfo({
      ...
      success: (info) => {
        ...
        tt.getFileSystemManager().readFile({
          ...
          fail: (err) => {
            ...
            tt.showToast({ title: "读取文件失败", icon: "none" });
            tt.hideLoading();
      ...
      fail: (err) => {
        ...
        tt.showToast({ title: "无法识别图片", icon: "none" });
        tt.hideLoading();
        this.resetAvatarUploadState();
      }
    });
  ...
  submitAvatar(imageData) {
    ...
    tt.request({
      ...
      success: (res) => {
        if (res.statusCode === 200 && res.data && res.data.code === "SUCCESS") {
          const newAvatar = (res.data && res.data.data && res.data.data.url) || "";
          this.setData({
            userInfo: {
              ...this.data.userInfo,
              avatar: newAvatar
          ...
          tt.showToast({ title: "头像已更新", icon: "success" });
        } else {
          tt.showToast({ title: (res.data && res.data.error) || "上传失败", icon: "none" });
        }
      },
      fail: (err) => {
  ...
  sendPasswordCode() {
    ...
    if (!phone) {
      tt.showToast({ title: "账号未绑定手机号", icon: "none" });
      return;
    }

    if (!/^\d{11}$/.test(phone)) {
      tt.showToast({ title: "手机号格式异常", icon: "none" });
      return;
    }
    ...
    tt.request({
      ...
      success: (res) => {
        if (res.statusCode === 200 && res.data && res.data.code === "SUCCESS") {
          ...
          tt.showToast({ title: (res.data && res.data.error) || "发送失败", icon: "none" });
        }
      },
      fail: (err) => {
  ...
  submitPasswordChange() {
    ...
    if (!/^[0-9]{6}$/.test(code)) {
      tt.showToast({ title: "请输入6位短信验证码", icon: "none" });
      return;
    }

    if (!newPassword || newPassword.length < 6) {
      tt.showToast({ title: "密码至少6位", icon: "none" });
      return;
    }

    if (newPassword !== confirmPassword) {
      tt.showToast({ title: "两次密码不一致", icon: "none" });
      return;
    }
    ...
    tt.request({
      ...
      success: (res) => {
        if (res.statusCode === 200 && res.data && res.data.code === "SUCCESS") {
          tt.showToast({ title: "密码已修改", icon: "success" });
          ...
          tt.showToast({ title: (res.data && res.data.error) || "修改失败", icon: "none" });
        }
      },
      fail: (err) => {
