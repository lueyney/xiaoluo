...
Page({
  ...
  onShow() {
    tt.setNavigationBarTitle({ title: "注册论文君" });
  },
  ...
  submit() {
    ...
    if (!codeValidation.valid) {
      tt.showToast({ title: codeValidation.message, icon: "none" });
      return;
    }
    ...
    if (!passwordValidation.valid) {
      tt.showToast({ title: passwordValidation.message, icon: "none" });
      return;
    }
    
    // 校验确认密码
    if (!confirmPassword) {
      tt.showToast({ title: "请再次输入密码", icon: "none" });
      return;
    }
    
    if (password !== confirmPassword) {
      tt.showToast({ title: "两次输入的密码不一致", icon: "none" });
      return;
    }
      ...
      success: res => {
        ...
        if (respCode === "SUCCESS" && data && data.token) {
          ...
          try {
            ...
            tt.showToast({ title: "注册失败，请重试", icon: "none" });
          }
        } else {
          tt.showToast({ 
  ...
  goLogin() {
    ...
    if (pages.length > 1) {
      ...
      tt.redirectTo({ url: "/pages/login/index" });
    }
  },
  onUnload() {
    if (this.countdownTimer) {
