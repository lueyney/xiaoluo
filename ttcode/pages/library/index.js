...
Page({
  ...
  onShow() {
    tt.setNavigationBarTitle({ title: "文档库" });
    // 清除可能的loading遮罩
    tt.hideLoading();
  ...
  loadDocuments() {
    ...
    documents.syncDocumentsFromServer()
      .then(result => {
        ...
        // 检查是否有新文档标记
        const hasNewDocs = tt.getStorageSync('hasNewDocuments');
        ...
        if (hasNewDocs) {
          ...
          setTimeout(() => {
            ...
            tt.setStorageSync('hasNewDocuments', false);
            tt.setStorageSync('newDocumentsCount', 0);
      ...
      .catch(err => {
        ...
        tt.showToast({ title: "网络异常，显示缓存数据", icon: "none" });
      });
  },
  calcTotalWords(list) {
  ...
  onCopyDoc(e) {
    ...
    if (!doc || !doc.content) {
      tt.showToast({ title: "内容为空", icon: "none" });
      return;
    }
    ...
    tt.setClipboardData({
      data: doc.content,
      success: () => {
        tt.showToast({ title: "内容已复制到剪贴板", icon: "success" });
      },
      fail: () => {
