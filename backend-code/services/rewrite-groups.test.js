...
describe('five-sentence rewrite groups', () => {
  ...
  test('source indexes preserve context adjacency after document filtering', () => {
    const sentences = [
      { text: '原句一。', pIdx: 0, sourceIndex: 0 },
      { text: '原句三。', pIdx: 0, sourceIndex: 2 }
    ];
    const results = ['改写一。', '原句三。'];
    expect(buildChainedRewriteContext(
      sentences,
      results,
      { idx: 1, sourceIndex: 2 },
      [{ idx: 0, sourceIndex: 0 }]
    )).toEqual({ previousRewrittenSentences: [], promptVariant: undefined });
  });
