...
Page({
  ...
  onShow() {
    tt.setNavigationBarTitle({ title: "幸运转盘" });
    ...
    tt.request({
      ...
      success: (res) => {
        if (res.data && res.data.code === "SUCCESS") {
          ...
          tt.showToast({ title: "今日已抽奖", icon: "none" });
        } else {
          this.setData({ isSpinning: false });
          tt.showToast({ title: "抽奖失败", icon: "none" });
        }
      },
      fail: (err) => {
  ...
  appendHistory(reward) {
    ...
    tt.setStorageSync("wheelHistory", list);
  },
  clearHistory() {
    tt.removeStorageSync("wheelHistory");
    this.setData({ history: [] });
  }
});
