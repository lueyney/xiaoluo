...
Page({
  ...
  onLoad(options) {
    ...
    if (!docId) {
      tt.showToast({ title: "文档ID无效", icon: "none" });
    ...
    if (!token) {
      ...
      setTimeout(() => {
        tt.redirectTo({ 
          ...
          fail: () => tt.reLaunch({ url: "/pages/login/index" })
        });
      }, 1500);
      return;
    }
  ...
  onExportWord() {
    const doc = this.data.document;
    if (!doc || !doc.id) {
      tt.showToast({ title: "文档信息异常", icon: "none" });
      return;
    }
    
    console.log('开始导出Word:', doc.title);
    tt.showLoading({ title: "正在生成Word...", mask: true });
    ...
    tt.request({
      ...
      success: (res) => {
        ...
        if (res.data && res.data.code === "SUCCESS") {
          ...
          // 使用 tt.env.USER_DATA_PATH 获取小程序专用临时目录
          // 清理文件名中的非法字符，确保文件名合法
          ...
          const localPath = `${tt.env.USER_DATA_PATH}/${fileName}`;
